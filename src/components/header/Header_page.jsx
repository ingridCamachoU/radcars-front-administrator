...
const Header_page = ({setIsOpenMenu}) => {
  ...
  return (
        <>
            <div className="tex-black flex top-0 right-0 w-full lg:justify-end p-4 gap-8 justify-between">
                <p className="flex lg:hidden w-[100px] h-[40px]">        
                    <img src={logo} alt="logo" />       
                </p>
                    
                <div className="flex gap-2 sm:gap-6 text-indigo-500 sm:flex-row flex-col w-9/12 justify-end">
                    <input 
                    type="search" 
                    placeholder="Search" 
                    ...
                    <p className="flex justify-end items-center">
                        ...
                        >   
                            <span onClick={handleClick}>
                                {darkMode ? 
                                <SunIcon  className='h-6 w-6 white text-indigo-500 cursor-pointer'/> : <MoonIcon  className='h-6 w-6 white text-indigo-500 cursor-pointer'/> } 
                            </span>
